...
3. **Typed payload detection** – quickly branch on inbound mail, delivery
   status, bounce, Kumo log, or form events.
...
> 📚 Need endpoint-by-endpoint details? See [`docs/endpoints.md`](./docs/endpoints.md)
> or the hosted documentation (link below).
...
The package ships as pure ESM with autogenerated type definitions. It expects an
Express-compatible runtime when you use the middleware helper, but the utility
...
const hasSharedSecret = verifySharedSecretHeader({
  ...
  provided: req.get("X-Kumo-Webhook-Secret"),
});
```
...
Kumo logs, and more. The guards make branching safe and ergonomic:
...
| `verifyVenmailSignature(options)` | Low-level SHA-256 HMAC validation (mirrors `DeliverOrganizationIntegrationEvent`). |
| `verifySharedSecretHeader(options)` | Constant-time comparison for shared-secret headers (e.g. `X-Kumo-Webhook-Secret`). |
...
All exported interfaces live in `src/types.ts` and are re-exported from the
package root for IDE autocompletion.

## Endpoints & payload contracts

The Venmail backend exposes several webhook surfaces across Laravel controllers.
Rather than duplicating them here, the package maintains a dedicated document
that you can keep alongside your integration playbooks. Jump to
[`docs/endpoints.md`](./docs/endpoints.md) for:

- Mail ingestion routes (`/api/v1/mails/org/{id}`)
- Campaign delivery + bounce routes
- Kumo log hook routes and shared-secret headers
- Form webhook CRUD routes and emitted events (`form.response.*`)
- Stripe + payment secondary webhooks
- Organization integration dispatch pipeline

Each entry cites the source file/lines inside `mailer_web` so you can audit the
implementation details when upgrades land.
